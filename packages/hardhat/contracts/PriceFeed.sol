...
contract PriceFeed is MainDemoConsumerBase {
	...
	function validateTimestamp(
		...
		if (blockTimestampMilliseconds > receivedTimestampMilliseconds) {
			require(
				blockTimestampMilliseconds - receivedTimestampMilliseconds <=
					maxTimestampDiffMilliseconds,
				"Timestamp too old"
			);
		}
		// Check if timestamp is too far in the future
			...
			require(
				receivedTimestampMilliseconds - blockTimestampMilliseconds <=
					maxTimestampDiffMilliseconds,
	...
	function getMultiplePrices()
		public
		view
		returns (uint256 ethPrice, uint256 btcPrice)
	{
